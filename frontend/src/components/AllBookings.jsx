...
const AllBookings = () => {
    ...
    const { data: bookingsData, loading, error, refetch } = useFetch(`/bookings?page=${page}&limit=${limit}`)
    ...
    const handleDelete = async (id) => {
        ...
        try {
            const response = await axios.delete(`${BACKEND_URL}/bookings/${id}`, { withCredentials: true })
            
            if (response.data.success) {
                showToast('success', response.data.message || 'Booking deleted successfully.')
                refetch()
            } else {
                showToast('error', response.data.message || 'Failed to delete booking.')
            }
        } catch (error) {
            console.error('Delete error:', error)
            showToast('error', error.response?.data?.message || 'Failed to delete booking. Please try again.')
        }
    }
    ...
    return (
        <div className='my-6 px-4'>
            <div className='mb-6 flex justify-between items-center flex-wrap gap-4'>
                <div>
                    <h1 className='text-3xl font-bold text-gray-800'>Bookings Table</h1>
                        ...
                        <div key={booking._id} className='bg-white rounded-lg shadow-md p-4 border border-gray-200'>
                            <div className='flex justify-between items-start mb-3'>
                                <div className='flex-1'>
                                    <h3 className='text-lg font-semibold text-gray-900'>{booking.customerId?.name || 'Unknown'}</h3>
                                    <p className='text-sm text-gray-600'>{booking.customerId?.email || 'No email'}</p>
                                </div>
                            ...
                            <div className='border-t border-gray-100 pt-3 space-y-2'>
                                <div className='flex justify-between items-center'>
                                    ...
                                    <span className='text-sm font-medium text-gray-900'>{booking.vehicleId?.name || 'Unknown'} ({booking.vehicleId?.brand || 'N/A'})</span>
                                </div>
                                <div className='flex justify-between items-center'>
                                    <span className='text-sm text-gray-600'>Start Date:</span>
                                    <span className='text-sm text-gray-900'>{formatDate(booking.startDate)}</span>
                                </div>
                                <div className='flex justify-between items-center'>
                                    <span className='text-sm text-gray-600'>End Date:</span>
                                    <span className='text-sm text-gray-900'>{formatDate(booking.endDate)}</span>
                                </div>
                    ...
                    <table className='w-full min-w-[800px]'>
                        <thead className='bg-gray-50 border-b border-gray-200'>
                            <tr>
                                ...
                                <th className='text-left p-4 font-semibold text-gray-700 text-sm uppercase tracking-wider'>Vehicle</th>
                                <th className='text-left p-4 font-semibold text-gray-700 text-sm uppercase tracking-wider'>Brand</th>
                                <th className='text-left p-4 font-semibold text-gray-700 text-sm uppercase tracking-wider'>Start Date</th>
                                <th className='text-left p-4 font-semibold text-gray-700 text-sm uppercase tracking-wider'>End Date</th>
                                <th className='text-left p-4 font-semibold text-gray-700 text-sm uppercase tracking-wider'>Total Price</th>
                            ...
                            {bookings?.length === 0 ? (
                                ...
                                bookings?.map((booking) => (
                                    <tr key={booking._id} className='hover:bg-gray-50 transition-colors'>
                                        ...
                                        <td className='p-4'>
                                            <div className='text-gray-600'>{booking.customerId?.email || 'No email'}</div>
                                        </td>
                                        <td className='p-4'>
                                            <div className='text-gray-900'>{booking.vehicleId?.name || 'Unknown'}</div>
                                        </td>
                                        <td className='p-4'>
                                            <div className='text-gray-600'>{booking.vehicleId?.brand || 'N/A'}</div>
                                        </td>
                                        <td className='p-4'>
                                            <div className='text-gray-600'>{formatDate(booking.startDate)}</div>
                                        </td>
                                        <td className='p-4'>
                                            <div className='text-gray-600'>{formatDate(booking.endDate)}</div>
                                        </td>
                                        <td className='p-4'>
